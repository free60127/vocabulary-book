...
6. 全部用中文写解析；题干可以中英混排。
7. **填空题（fill）只允许两种写法**，别的一律不要：
   ① **语境填空**：给一个英文句子、把要考的词挖成空格，答案就是那个词，
      但**题干里绝不能出现这个词**（包括不要在括号里提示"用 xxx 填空"）；
   ② **词形变化**：只有当这个词确有常用变形、且句子语法**强制**需要那个变形时，
      才可以在括号里给原形（如「用 decide 的适当形式填空」），此时**答案必须是变形后的形式**
      （decision / decided…），**不能与原形相同**。
   反面例子（**严禁**）：「Wearing a mask is ______ for all visitors.（用 mandatory 的适当形式填空）」答案是 mandatory
   —— 题干把答案词告诉了学生、答案又是原词，这题等于白送，做了没有任何意义。
   如果一句话只能考"这个词是什么意思"，就不要用填空，改用 choice。`;
/**
 * 出题的用户消息。
